...
La universidad ABC está cambiando el sistema de calificación tradicional. Se propone que el alumno sea evaluado de manera constante. Para ello, al finalizar cada sesión de clase el profesor tomará una evaluación de los temas vistos en dicha sesión y de una duración máxima de quince minutos. Si el alumno falto a dicha sesión su nota para esa evaluación será de cero (0).
De acuerdo a ello se ha determinado que el alumno rendirá 13 pruebas que equivalen a las trece semanas de clase. Por ello, se le solicita lo siguiente:

a)	Desarrollar un subprograma que reciba las notas promedio final de los alumnos y determine el promedio de todo el salón
b)	Desarrollar un subprograma que teniendo como base las 13 notas de un alumno, nos retorne el promedio final obtenido. El promedio final se calculará tomando las 10 mejores notas obtenidas durante las 13 sesiones que tiene el curso.
c)	Desarrollar un subprograma que determine reciba las notas promedio final de todos los alumnos y determine quién fue el mejor alumno de la clase. 
...
def diez_elementos(notas)
  diez_mejores_notas = []
  temporal = notas.sort.reverse
  for i in 0...10
    diez_mejores_notas.push(temporal[i])  
  end
  return diez_mejores_notas
end

def calcularpromedioalumno(notas)
    total = 0
	diez_notas = diez_elementos(notas)
	for i in 0...10
	  total = total + diez_notas[i]
	end
