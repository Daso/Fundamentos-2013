...
Segundo, los plazos por el cliente se realizan en meses, por ejemplo: 6, 12 o 18 meses. Que equivaldrán al número de cuotas que deberá pagar el cliente.

Tercero, si el cliente califica al crédito según el punto primero, se solicita calcular el monto de la cuota a pagar que consiste en dividir el monto del préstamo total entre el número de cuotas. La cuota ya contempla el interés a pagar que se aplica según el monto del préstamo de acuerdo a la siguiente tabla:
...
Ejemplo:
Si el cliente solicita S/. 1,000.00 le corresponde la tasa de interés de 45% según la tabla. Si solicitó pagarlo a 12 meses la cuota seria: (1000 * 1.45) / 12 = 120.83
...
def calificacliente(tipo, ubicacion, ingresos, deudas)
  ...
  if tipo == 'B' && ubicacion == 'C' && deudas <= limite
    return true
  else
    return false
  end
...
def calculaCuota(tipo,ubicacion,ingresos,deudas,monto,meses)
  tasa =  devuelveTasa(monto)
  if calificacliente(tipo, ubicacion, ingresos, deudas)
    return (monto * (1 + tasa / 100) / meses).round(2)
  else
    return 0.0
  end
end
